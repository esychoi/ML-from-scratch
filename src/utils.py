...
def plot_frontiere(data,f,step=20):
	...
	plt.contourf(x,y,f(grid).reshape(x.shape),levels=[-1,0,1])
...
def plot_digits(autoenc,X,X_noisy,n=5,prediction=False):
	"""
	Choose n vectors from X randomly and imshow them
	"""
	np.random.seed(10)
	ids = np.random.choice(X_noisy.shape[0],size=n)
	to_plot = X[ids]
	to_plot_noisy = X_noisy[ids]
	if prediction:
		decoded = autoenc.predict(to_plot_noisy)
		plt.figure(figsize=(15, 5))
		for i in range(n):
			ax = plt.subplot(3, n, i + 1)
			plt.imshow(to_plot[i].reshape(28, 28),cmap='gray')
			ax.get_xaxis().set_visible(False)
			ax.get_yaxis().set_visible(False)

			ax = plt.subplot(3, n, i + 1 + n)
			plt.imshow(to_plot_noisy[i].reshape(28, 28),cmap='gray')
			ax.get_xaxis().set_visible(False)
			ax.get_yaxis().set_visible(False)
